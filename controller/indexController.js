...
module.exports.post_detail_get = (req, res, next) => {
  Post.findOne({ _id: req.params.postId })
    ...
    .catch((err) => {
      res
        .status(404)
        .json({ message: `Post with id: ${req.params.postId} Not Found`, err });
    });
};

module.exports.comments_list_get = (req, res, next) => {
  Comment.find({ blog_id: req.params.postId })
    .sort({ timestamp: 1 })
    .exec()
    .then((results) => {
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      comment
        ...
        .then(() => {
          res.json({
            message: `Comment by ${comment.email} saved successfully`,
          });
        })
        .catch((err) => {
